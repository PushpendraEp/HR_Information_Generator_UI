...
*/
  ...
  changeMonthType(resData: any, type: string, singleData = false) {

    let data = [...resData];

    if (type == 'number to month') {
      for (let i = 0; i < data.length; i++) {
        this.switch_case_for_change_number_to_month_type(data, i);
      }
    }
    else {
      for (let i = 0; i < data.length; i++) {
        this.switch_case_for_change_month_to_number_type(data, i);
      }
    }
  ...
  switch_case_for_change_month_to_number_type(data: any, i: number) {
    switch (true) {
